...
mod tests {
    ...
    fn test_create_node() {
        ...
        assert!(query_ctx
            .get_world_transform(GraphNodeId::Node(child_id))
            .is_some());
    }
    ...
    fn test_data_node_mapping() {
        ...
        let data_node_id = CanvasNodeId::new(123);
        ...
        assert_eq!(
            mod_ctx.get_data_node_id(scene_graph_id.clone()),
            Some(data_node_id)
        );
        assert_eq!(
            mod_ctx.get_scene_node_id(data_node_id),
            Some(scene_graph_id.clone())
        );
