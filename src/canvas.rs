...
impl LunaCanvas {
    ...
    pub fn add_node(&mut self, mut node: FrameNode, parent_id: Option<NodeId>, cx: &mut Context<Self>) -> NodeId {
        let node_id = node.id();
    ...
    pub fn add_child_to_parent(&mut self, parent_id: NodeId, child_id: NodeId, cx: &mut Context<Self>) -> bool {
        ...
        let scene_updated = self.scene_graph.update(cx, |sg, _| {
            ...
            match (parent_scene_id, child_scene_id) {
                (Some(parent_scene), Some(child_scene)) => {
                    sg.add_child(parent_scene, child_scene)
                },
                _ => false
            }
        });
        ...
        if let Some(parent_id) = parent_id {
            ...
            let scene_updated = self.scene_graph.update(cx, |sg, _| {
                ...
                match child_scene_id {
                    Some(child_scene) => {
                        sg.add_child(self.canvas_node, child_scene)
                    },
                    _ => false
                }
            });
