//! Input element for single-line and multi-line text editing.

use gpui::{
    fill, point, px, relative, Action, App, Bounds, ContentMask, Context, CursorStyle,
...
use super::handler::ElementInputHandler;
use super::state::{InputLineLayout, InputState};
use super::bidi::TextDirection;

const CURSOR_WIDTH: f32 = 2.0;
const MARKED_TEXT_UNDERLINE_THICKNESS: f32 = 2.0;

/// Input colors for theming.
#[derive(Clone, Debug)]
...
impl Default for InputColors {
    fn default() -> Self {
        Self {
            selection: gpui::hsla(0.58, 0.9, 0.5, 0.3),
            cursor: gpui::hsla(0.58, 0.9, 0.5, 1.0),
            placeholder: gpui::hsla(0.0, 0.0, 0.5, 1.0),
        }
    }
}

/// Creates a new single-line Input element.
#[track_caller]
pub fn input(input_state: &Entity<InputState>, cx: &App) -> Input {
    Input::new(input_state, false, cx)
}

/// Creates a new multi-line Input element (text area).
#[track_caller]
...
impl Input {
    ...
    fn new(input_state: &Entity<InputState>, multiline: bool, cx: &App) -> Self {
        ...
        input.key_context(super::bindings::INPUT_CONTEXT).track_focus(&focus_handle)
    }
    ...
    pub fn selection_color(mut self, color: impl Into<Hsla>) -> Self {
        self.colors.selection = color.into();
        self
    }

    /// Sets the cursor color.
    pub fn cursor_color(mut self, color: impl Into<Hsla>) -> Self {
        self.colors.cursor = color.into();
        self
    }
    ...
    fn register_actions(&mut self) {
        ...
        register_action(&mut self.interactivity, &self.input, InputState::select_left);
        register_action(&mut self.interactivity, &self.input, InputState::select_right);
        register_action(&mut self.interactivity, &self.input, InputState::select_up);
        register_action(&mut self.interactivity, &self.input, InputState::select_down);
        ...
        register_action(&mut self.interactivity, &self.input, InputState::move_to_beginning);
        register_action(&mut self.interactivity, &self.input, InputState::move_to_end);
        register_action(&mut self.interactivity, &self.input, InputState::select_to_beginning);
        register_action(&mut self.interactivity, &self.input, InputState::select_to_end);
        ...
        register_action(&mut self.interactivity, &self.input, InputState::select_word_left);
        register_action(&mut self.interactivity, &self.input, InputState::select_word_right);
        ...
        register_action(&mut self.interactivity, &self.input, InputState::delete_word_left);
        register_action(&mut self.interactivity, &self.input, InputState::delete_word_right);
        register_action(&mut self.interactivity, &self.input, InputState::delete_to_beginning_of_line);
        register_action(&mut self.interactivity, &self.input, InputState::delete_to_end_of_line);
        ...
        self.interactivity.on_action::<Escape>(|_action, window, _cx| {
            window.blur();
        });
    }
}
...
fn mouse_down(input: Entity<InputState>, bounds: Bounds<Pixels>, multiline: bool, window: &mut Window) {
    window.on_mouse_event(move |event: &MouseDownEvent, phase, window, cx| {
        ...
        input.update(cx, |input, cx| {
            ...
            input.on_mouse_down(text_position, event.click_count, event.modifiers.shift, window, cx);
        });
    });
}
...
fn mouse_move(input: Entity<InputState>, bounds: Bounds<Pixels>, multiline: bool, window: &mut Window) {
...
fn paint_multiline_selection(
    ...
    for line in line_layouts {
        ...
        if !is_line_visible(line_y, line_height, line.visual_line_count, bounds.size.height) {
            continue;
        }
...
fn paint_singleline(
    ...
    if state.content.is_empty() {
        if let Some(placeholder_str) = placeholder {
            if !placeholder_str.is_empty() {
                paint_singleline_placeholder(placeholder_str, bounds, text_style, colors.placeholder, window, cx);
            }
        }
    } else {
...
fn paint_singleline_selection(
    ...
    let start_x = state.x_for_index(state.selected_range.start) - state.scroll_offset + alignment_offset;
    let end_x = state.x_for_index(state.selected_range.end) - state.scroll_offset + alignment_offset;
    ...
    window.paint_quad(fill(
        Bounds::from_corners(
            ...
            point(bounds.left() + end_x, bounds.top() + y_offset + state.line_height),
        ),
        selection_color,
    ));
}
