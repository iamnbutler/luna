...
FRAME1_ID=$(echo "$FRAME1" | extract_id)
...
if [ -n "$FRAME1_ID" ]; then
    ...
    for i in 1 2 3 4; do
        ...
        CHILD_ID=$(echo "$CHILD" | extract_id)
        if [ -n "$CHILD_ID" ]; then
    ...
    echo "  ERROR: Failed to create frame"
fi
sleep 0.5
...
FRAME2_ID=$(echo "$FRAME2" | extract_id)
...
FRAME3_ID=$(echo "$FRAME3" | extract_id)
...
FRAME4_ID=$(echo "$FRAME4" | extract_id)
